...
! function (root, factory) {
    "use strict";
    "function" == typeof define && define.amd ? define([], factory) : "object" == typeof module && module.exports ? module.exports = factory() : root.pipwerks = factory()
}(this, (function () {
    "use strict";
    var pipwerks = {
        UTILS: {},
    ...
    return pipwerks.SCORM.isAvailable = function () {
        return !0
    }, pipwerks.SCORM.API.find = function (win) {
        for (var API = null, findAttempts = 0, findAttemptLimit = 500, traceMsgPrefix = "SCORM.API.find", trace = pipwerks.UTILS.trace, scorm = pipwerks.SCORM; !win.API && !win.API_1484_11 && win.parent && win.parent != win && findAttempts <= 500;) findAttempts++, win = win.parent;
        if (scorm.version) switch (scorm.version) {
            case "2004":
                win.API_1484_11 ? API = win.API_1484_11 : trace("SCORM.API.find: SCORM version 2004 was specified by user, but API_1484_11 cannot be found.");
                break;
            case "1.2":
                win.API ? API = win.API : trace("SCORM.API.find: SCORM version 1.2 was specified by user, but API cannot be found.")
        } else win.API_1484_11 ? (scorm.version = "2004", API = win.API_1484_11) : win.API && (scorm.version = "1.2", API = win.API);
        return API ? (trace("SCORM.API.find: API found. Version: " + scorm.version), trace("API: " + API)) : trace("SCORM.API.find: Error finding API. \nFind attempts: " + findAttempts + ". \nFind attempt limit: 500"), API
    }, pipwerks.SCORM.API.get = function () {
        var API = null,
            win = window,
            scorm = pipwerks.SCORM,
            find = scorm.API.find,
            trace = pipwerks.UTILS.trace;
        return !(API = find(win)) && win.parent && win.parent != win && (API = find(win.parent)), !API && win.top && win.top.opener && (API = find(win.top.opener)), !API && win.top && win.top.opener && win.top.opener.document && (API = find(win.top.opener.document)), API ? scorm.API.isFound = !0 : trace("API.get failed: Can't find the API!"), API
    }, pipwerks.SCORM.API.getHandle = function () {
        var API = pipwerks.SCORM.API;
        ...
        var success = !1,
            ...
            debug = scorm.debug,
            traceMsgPrefix = "SCORM.connection.initialize ";
        if (trace("connection.initialize called."), scorm.connection.isActive) trace(traceMsgPrefix + "aborted: Connection already active.");
        else {
            var API = scorm.API.getHandle(),
                errorCode = 0;
            if (API) {
                ...
                if (success)
                    if (null !== (errorCode = debug.getCode()) && 0 === errorCode) {
                        if (scorm.connection.isActive = !0, scorm.handleCompletionStatus && (completionStatus = scorm.status("get"))) {
                            switch (completionStatus) {
                                case "not attempted":
                                case "unknown":
                                    scorm.status("set", "incomplete")
        ...
        if (scorm.connection.isActive) {
            ...
            if (API) {
                if (scorm.handleExitMode && !exitStatus)
                    if ("completed" !== completionStatus && "passed" !== completionStatus) switch (scorm.version) {
                        case "1.2":
                            success = scorm.set("cmi.core.exit", "suspend");
                            break;
                        case "2004":
                ...
                if (success = "1.2" !== scorm.version || scorm.save()) {
                    switch (scorm.version) {
                        case "1.2":
                            success = makeBoolean(API.LMSFinish(""));
                            break;
                    ...
                    success ? scorm.connection.isActive = !1 : trace(traceMsgPrefix + "failed. \nError code: " + (errorCode = debug.getCode()) + " \nError info: " + debug.getInfo(errorCode))
                }
            } else trace(traceMsgPrefix + "failed: API is null.")
        ...
        return trace(traceMsgPrefix + " value: " + value), success
    }, pipwerks.SCORM.data.save = function () {
        var success = !1,
            scorm = pipwerks.SCORM,
            trace = pipwerks.UTILS.trace,
        ...
        if (null !== action) {
            ...
            switch (action) {
                case "get":
                    ...
                    break;
                case "set":
                    null !== status ? success = scorm.data.set(cmi, status) : (success = !1, trace(traceMsgPrefix + ": status was not specified."));
                    break;
                default:
        ...
        return String(result)
    }, pipwerks.SCORM.init = pipwerks.SCORM.connection.initialize, pipwerks.SCORM.get = pipwerks.SCORM.data.get, pipwerks.SCORM.set = pipwerks.SCORM.data.set, pipwerks.SCORM.save = pipwerks.SCORM.data.save, pipwerks.SCORM.quit = pipwerks.SCORM.connection.terminate, pipwerks.UTILS.StringToBoolean = function (value) {
        var t;
        switch (typeof value) {
            case "object":
            case "string":
        ...
        pipwerks.debug.isActive && window.console && window.console.log && window.console.log(msg)
    }, pipwerks
}));